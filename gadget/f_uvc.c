...
uvc_function_set_alt(struct usb_function *f, unsigned interface, unsigned alt)
{
	...
	if (!bulk_streaming_ep) {
		...
		case UVC_STATE_STREAMING:
			printk("set alt:  is streaming  do nothing!\n");
#if 0
			if (uvc->video.ep && uvc->video.ep->driver_data) {
...
//	if (uvc->state == UVC_STATE_STREAMING)
	{
		INFO(f->config->cdev, "UVC_EVENT_STREAMOFF\n");
		memset(&v4l2_event, 0, sizeof(v4l2_event));
		v4l2_event.type = UVC_EVENT_STREAMOFF;
		v4l2_event_queue(uvc->vdev, &v4l2_event);
	}
	...
	INFO(f->config->cdev, "UVC_EVENT_DISCONNECT\n");
	memset(&v4l2_event, 0, sizeof(v4l2_event));
	...
	if (uvc->video.ep->driver_data) {
			INFO(f->config->cdev, "usb_ep_disable  video ep\n\n");
			usb_ep_disable(uvc->video.ep);

...
int sfifo_reset(struct sfifo_des_s *sfifo_des_p)
{
	int ret = 0;
	struct sfifo_s *sfifo = NULL;

	ret = pthread_mutex_lock(&(sfifo_des_p->active_list.lock_mutex));
	if (ret != 0) {
		ALOGD("lock active_list.lock_mutex error\n");
		goto FAILED;
	}
	ret = pthread_mutex_lock(&(sfifo_des_p->free_list.lock_mutex));
	if (ret != 0) {
		ALOGD("lock free_list.lock_mutex error\n");
		goto EXIT;
	}

...
static void ClearDpfProfileList(  List *l )
{
    if ( !ListEmpty( l ) )
    {
        ...
        while ( pDpfProfile )
        {
            ...
            if (pDpfProfile->SharpeningLevelCurve.pSensorGain) {
                free(pDpfProfile->SharpeningLevelCurve.pSensorGain);
            }
...
/******************************************************************************
 * ValidateGammaoutData
 *****************************************************************************/
static inline RESULT ValidateGammaoutData( CamCalibGammaOut_t  *pGammaOutData )
{
    TRACE( CAM_CALIBDB_API_INFO, "%s (enter)\n", __FUNCTION__ );

    if ( NULL == pGammaOutData )
    {
        return ( RET_NULL_POINTER );
    }
...
static RESULT ClearContext( CamCalibDbContext_t *pCamCalibDbCtx )
{
	...
	if(pCamCalibDbCtx->pGammaOut)
	{
		free(pCamCalibDbCtx->pGammaOut);
	}
...
)
{
	...
	pCamCalibDbCtx->OTPInfo.nRG_Ratio_Typical = pMeta->OTPInfo.nRG_Ratio_Typical;
	pCamCalibDbCtx->OTPInfo.nBG_Ratio_Typical = pMeta->OTPInfo.nBG_Ratio_Typical;
    TRACE( CAM_CALIBDB_API_WARN, "%s RG=%d,BG=%d\n", __FUNCTION__ ,pCamCalibDbCtx->OTPInfo.nRG_Ratio_Typical,pCamCalibDbCtx->OTPInfo.nBG_Ratio_Typical);	
    TRACE( CAM_CALIBDB_API_INFO, "%s (exit)\n", __FUNCTION__ );
...
)
{
    ...
    if ( NULL == pNewAwbGlobal )
    {
		...
		if(pAddAwbGlobal->AwbFade2Parm.pMinCRegionMax){
		...
		if(pAddAwbGlobal->AwbFade2Parm.pMinCRegionMin){
		...
		if(pAddAwbGlobal->AwbFade2Parm.pMaxYRegionMax){
		...
		if(pAddAwbGlobal->AwbFade2Parm.pMaxYRegionMin){
		...
		if(pAddAwbGlobal->AwbFade2Parm.pMinYMaxGRegionMax){
		...
		if(pAddAwbGlobal->AwbFade2Parm.pMinYMaxGRegionMin){
		...
		if(pAddAwbGlobal->AwbFade2Parm.pRefCb){
		...
		if(pAddAwbGlobal->AwbFade2Parm.pRefCr){
...
)
{
    ...
    TRACE( CAM_CALIBDB_API_INFO, "%s CXF (enter)\n", __FUNCTION__ );
    ...
    result = ValidateGammaoutData( pAddGammaOut );
    if ( result != RET_SUCCESS )
    {
        return ( result );
    }
    ...
    /* check if data already exists */
    if ( NULL != pCamCalibDbCtx->pGammaOut )
    {
        return ( RET_INVALID_PARM );
    }

    /* finally allocate, copy & add data */
    CamCalibGammaOut_t *pNewGammaOut = malloc( sizeof(CamCalibGammaOut_t) );
    if ( NULL == pNewGammaOut )
    {
        return ( RET_OUTOFMEM );
    }
    MEMCPY( pNewGammaOut, pAddGammaOut, sizeof(CamCalibGammaOut_t) );

    pCamCalibDbCtx->pGammaOut = pNewGammaOut;
	TRACE( CAM_CALIBDB_API_INFO, "%s xscale=%d \n", __FUNCTION__,pNewGammaOut->Curve.xScale);
	TRACE( CAM_CALIBDB_API_INFO, "%s y0=%d,y16=%d \n", __FUNCTION__,pNewGammaOut->Curve.GammaY[0],pNewGammaOut->Curve.GammaY[16]);
    TRACE( CAM_CALIBDB_API_INFO, "%s (exit)\n", __FUNCTION__ );
	TRACE( CAM_CALIBDB_API_ERROR, "%s xscale=%d \n", __FUNCTION__, pCamCalibDbCtx->pGammaOut->Curve.xScale);
	TRACE( CAM_CALIBDB_API_ERROR, "%s y0=%d,y16=%d \n", __FUNCTION__, pCamCalibDbCtx->pGammaOut->Curve.GammaY[0], pCamCalibDbCtx->pGammaOut->Curve.GammaY[16]);
...
)
{
    ...
    if ( NULL == pCamCalibDbCtx )
    {
     TRACE( CAM_CALIBDB_API_ERROR, "%s NULL == pCamCalibDbCtx\n", __FUNCTION__ );
        return ( RET_WRONG_HANDLE );
    }

    if ( NULL == ppGammaOut )
    {
     TRACE( CAM_CALIBDB_API_ERROR, "%s NULL == ppGammaOut\n", __FUNCTION__ );
        return ( RET_INVALID_PARM );
    }

    /* return reference to Gamma Out configuration */
    *ppGammaOut = pCamCalibDbCtx->pGammaOut;

	TRACE( CAM_CALIBDB_API_INFO, "%s xscale=%d \n", __FUNCTION__, pCamCalibDbCtx->pGammaOut->Curve.xScale);
	TRACE( CAM_CALIBDB_API_INFO, "%s y1=%d,y16=%d \n", __FUNCTION__, pCamCalibDbCtx->pGammaOut->Curve.GammaY[1], pCamCalibDbCtx->pGammaOut->Curve.GammaY[16]);
...
)
{
	...
	*pOTPInfo = (pCamCalibDbCtx->OTPInfo.nRG_Ratio_Typical)<<16 | (pCamCalibDbCtx->OTPInfo.nBG_Ratio_Typical);

    ...
    if(pIsiCamDrvConfig->pfIsiGetSensorI2cInfo){
        ...
        if(err==RK_RET_SUCCESS)
        {
        	if(pIsiCamDrvConfig->IsiSensor.pIsiSensorCaps->SensorOutputMode == ISI_SENSOR_OUTPUT_MODE_RAW){
				...
				if (!strlen(pSensorInfo->mModuleName) && !strlen(pSensorInfo->mLensName)) {
					LOGW("Camera module name and len name haven't configured, so used default tuning file!");
					default_tuning = true;
				}else{
					if(strlen(pSensorInfo->mModuleName)){
				...
				if (default_tuning == false) {
				    if (0 != access(pLoadSensorInfo->mSensorXmlFile, R_OK)) {
				          LOGW("The tuning file:%s isn't exist! so used default tuning file",pLoadSensorInfo->mSensorXmlFile);
				          default_tuning = true;
				    }
				}
				...
				if (0 != access(pLoadSensorInfo->mSensorXmlFile, R_OK)) {
					LOGE("Fatal error, tunning file %s is not exist!", pLoadSensorInfo->mSensorXmlFile);
				}
				//

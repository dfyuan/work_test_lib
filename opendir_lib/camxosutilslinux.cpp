...
#include <cutils/open_memstream.h>  // memory stream
#include <sync/sync.h>
#include <sys/prctl.h>
    ...
    SIZE_T  offset)
{
    ...
    if ((bufferLength > 0) && (bufferFD > 0))
    {
        ...
        if (MAP_FAILED == pMem)
        {
            CAMX_LOG_ERROR(CamxLogGroupCSL, "mmap() failed! errno %d", errno);
            pMem = NULL;
        }
    }
    ...
    const CHAR* pResourceName)
{
    ...
    {
        TraceMessageF(CamxLogGroupSync, "Mutex Init");
        m_validMutex = TRUE;
    }
    else
    {
...
VOID Mutex::Lock()
{
    TraceSyncBeginF(CamxLogGroupSync, "%s: Mutex::Lock", m_pResourceName);
    pthread_mutex_lock(&m_mutex);
    TraceAsyncBeginF(CamxLogGroupSync, Utils::VoidPtrToUINT64(GetNativeHandle()), "%s: Mutex Held", m_pResourceName);
    TraceSyncEnd(CamxLogGroupSync);
}
...
VOID Mutex::Unlock()
{
    ...
    TraceAsyncEndF(CamxLogGroupSync, Utils::VoidPtrToUINT64(GetNativeHandle()), "%s: Mutex Held", m_pResourceName);
}
...
{
    TraceSyncBeginF(CamxLogGroupSync, "%s: RWLock::ReadLock", m_pResourceName);
    pthread_rwlock_rdlock(&m_readWriteLock);
    INT lockCount = CamxAtomicInc(&m_lockCount);
    TraceInt32F(CamxLogGroupSync, lockCount, "%s: RWLock::LockCount" , m_pResourceName);
    TraceSyncEnd(CamxLogGroupSync);
}
...
{
    TraceSyncBeginF(CamxLogGroupSync, "%s: RWLock::WriteLock", m_pResourceName);
...
BOOL ReadWriteLock::TryReadLock()
{
    ...
    TraceSyncBeginF(CamxLogGroupSync, "%s: RWLock::TryReadLock", m_pResourceName);
    if (0 == pthread_rwlock_tryrdlock(&m_readWriteLock))
    {
        isAcquired = TRUE;
        INT lockCount = CamxAtomicInc(&m_lockCount);
        TraceInt32F(CamxLogGroupSync, lockCount, "%s: RWLock::LockCount" , m_pResourceName);
        TraceMessageF(CamxLogGroupSync, "%s: Acquired lock", m_pResourceName);
    }
    TraceSyncEnd(CamxLogGroupSync);
...
BOOL ReadWriteLock::TryWriteLock()
{
    ...
    TraceSyncBeginF(CamxLogGroupSync, "%s: RWLock::TryWriteLock", m_pResourceName);
...
VOID ReadWriteLock::Unlock()
{
    ...
    TraceInt32F(CamxLogGroupSync, lockCount, "%s: RWLock::LockCount" , m_pResourceName);
    TraceMessageF(CamxLogGroupSync, "%s: Unlock", m_pResourceName);
...
{
    TraceSyncBeginF(CamxLogGroupSync, "Condition::Wait %s", m_pResource);
    CAMX_ASSERT(NULL != phMutex);

    INT         rc      = 0;
    CamxResult  result  = CamxResultEFailed;

    TraceAsyncEndF(CamxLogGroupSync, Utils::VoidPtrToUINT64(phMutex), "ConditionUnlock %s", m_pResource);
    rc = pthread_cond_wait(&m_conditionVar, phMutex);
    TraceAsyncBeginF(CamxLogGroupSync, Utils::VoidPtrToUINT64(phMutex), "ConditionLock %s", m_pResource);
...
CamxResult Condition::TimedWait(
    ...
    UINT            timeoutMilliseconds)
{
    TraceSyncBeginF(CamxLogGroupSync, "Condition::TimedWait %s Timeout: %d", m_pResource, timeoutMilliseconds);
    CAMX_ASSERT(NULL != phMutex);
    ...
    TraceAsyncEnd(CamxLogGroupSync, SCOPEEventOsUtilsMutexLock, Utils::VoidPtrToUINT64(phMutex));
    waitResult = pthread_cond_timedwait(&m_conditionVar, phMutex, &timeout);
    TraceAsyncBegin(CamxLogGroupSync, SCOPEEventOsUtilsMutexLock, Utils::VoidPtrToUINT64(phMutex));
    if (waitResult != 0)
    {
        // Check errno for reason for failure
        if (ETIMEDOUT == waitResult)
...
{
    CAMX_LOG_PERF_INFO(CamxLogGroupSync, "Condition::Signal %s", m_pResource);
...
{
    CAMX_LOG_PERF_INFO(CamxLogGroupSync, "Condition::Broadcast %s", m_pResource);
...
Gralloc::Gralloc()
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupCore, "Gralloc setup failed!!");
    }
}
...
Gralloc* Gralloc::GetInstance()
{
    ...
    return &s_GrallocSingleton;
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Gralloc::Alloc
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CamxResult Gralloc::Alloc(
    UINT32           width,
    UINT32           height,
    CamX::Format     format,
    buffer_handle_t* phAllocatedBuffer,
    UINT32*          pStride)
{
    INT32  result           = GRALLOC1_ERROR_NONE;
    UINT64 producerFlags    = GRALLOC1_PRODUCER_USAGE_CAMERA;
    UINT64 consumerFlags    = GRALLOC1_CONSUMER_USAGE_CAMERA | GRALLOC1_PRODUCER_USAGE_CPU_READ;
    UINT32 grallocFormat;

    gralloc1_buffer_descriptor_t gralloc1BufferDescriptor = {0};

    CAMX_LOG_ERROR(CamxLogGroupCore, "Krupal gralloc phAllocatedBuffer  %p", phAllocatedBuffer);

    switch (format)
    {
        case CamX::Format::YUV420NV12:
        case CamX::Format::YUV420NV21:
            grallocFormat = HAL_PIXEL_FORMAT_YCbCr_420_888;
            break;
        default:
            CAMX_LOG_ERROR(CamxLogGroupCore, "Unsupported Gralloc format");
    ...
    if (GRALLOC1_ERROR_NONE == result)
    {
        result = m_grallocInterface.SetFormat(m_pGralloc1Device, gralloc1BufferDescriptor, grallocFormat);
    }

    if (GRALLOC1_ERROR_NONE == result)
    {
        result = m_grallocInterface.SetProducerUsage(m_pGralloc1Device, gralloc1BufferDescriptor, producerFlags);
    }

    if (GRALLOC1_ERROR_NONE == result)
    {
        result = m_grallocInterface.SetConsumerUsage(m_pGralloc1Device, gralloc1BufferDescriptor, consumerFlags);
    }

    if (GRALLOC1_ERROR_NONE == result)
    {
        result = m_grallocInterface.Allocate(m_pGralloc1Device, 1, &gralloc1BufferDescriptor, phAllocatedBuffer);
    }

    if (GRALLOC1_ERROR_NONE == result)
    {
        result = m_grallocInterface.GetStride(m_pGralloc1Device, *phAllocatedBuffer, pStride);
    }
...
// Gralloc::Free
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CamxResult Gralloc::Free(
    buffer_handle_t hAllocatedBuffer)
{
    m_grallocInterface.Release(m_pGralloc1Device, hAllocatedBuffer);
    CAMX_LOG_ERROR(CamxLogGroupCore, "Krupal free gralloc %p", hAllocatedBuffer);
